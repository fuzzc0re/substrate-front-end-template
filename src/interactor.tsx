...
type InterxType = 'QUERY' | 'EXTRINSIC' | 'CONSTANT' | 'RPC';
...
interface IInteractorSubmit extends IInteractor {
  setStatus: any;
  attrs: {
    interxType: InterxType;
    palletRpc: string;
    callable: string;
    inputParams: any[];
    paramFields: any[];
...
const InteractorSubmit: FC<IInteractorSubmit> = (
  props,
  { attrs: { interxType } }
) => {
  if (interxType === 'QUERY') {
...
const Main: FC<{ accountPair?: KeyringPair }> = ({ accountPair }) => {
  ...
  const [interxType, setInterxType] = useState<InterxType>('EXTRINSIC');
  ...
  const OptionalLabel: FC<{ show: boolean }> = ({ show }) => {
    if (show) {
      return (
        <Label
          basic
        ...
        {paramFields.map((paramField, ind) => (
          <Form.Field key={`${paramField.name}-${paramField.type}`}>
            <Input
              ...
              onChange={onPalletCallableParamChange}
            />
            <OptionalLabel show={paramField.optional != null} />
          </Form.Field>
        ))}
...
const Interactor: FC<IInteractor> = props => {
